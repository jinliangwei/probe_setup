...
if __name__ == "__main__":
    if len(sys.argv) != 3 and len(sys.argv) != 2:
        print "Usage: %s num-hosts host-suffix || %s shutdown" % sys.argv[0]
        exit(1)
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "shutdown":
        ...
        ssh(master_ip, "/users/jinlianw/spark-2.1.0-bin-hadoop2.7/sbin/stop-slaves.sh")
        ssh(master_ip, "/users/jinlianw/spark-2.1.0-bin-hadoop2.7/sbin/stop-master.sh")
        ssh(master_ip, "/users/jinlianw/hadoop-2.7.3/sbin/stop-dfs.sh")
        exit(1)
    ...
    os.system(gen_hosts + " %s %s" % (str(num_hosts), host_suffix))
